...
const List = ({ permission, onDeleteItem, onEditItem, isMotion, location, ...tableProps }) => {
  ...
  const handleMenuClick = (record, e) => {
    if (e.key === Operation.UPDATE) {
      ...
      confirm({
        title: intl.get('user_delete_confirm'),
        onOk () {
  ...
  const columns = [
	{
      title: intl.get('user_username').defaultMessage('User Name'),
      dataIndex: 'username',
      key: 'username',
      render: (text, record) => <Link to={`user/${record.id}`}>{text}</Link>,
      sorter: true,
    }, {
      title: intl.get('user_nickName').defaultMessage('Nick Name'),
      dataIndex: 'nickName',
      key: 'nickName',
    }, {
      title: intl.get('user_age').defaultMessage('Age'),
      dataIndex: 'age',
      key: 'age',
      sorter: true,
    }, {
      title: intl.get('user_gender').defaultMessage('Gender'),
      dataIndex: 'isMale',
      key: 'isMale',
      filters: [{
      ...
      title: intl.get('user_phone').defaultMessage('Phone'),
      dataIndex: 'phone',
      key: 'phone',
    }, {
      title: intl.get('user_email').defaultMessage('Email'),
      dataIndex: 'email',
      key: 'email',
    }, {
      title: intl.get('user_address').defaultMessage('Address'),
      dataIndex: 'address',
      key: 'address',
    }, {

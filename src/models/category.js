...
export default modelExtend(pageModel, {
  ...
  state: {
    ...
    modalType: 'create',
    selectedRowKeys: [],
    isMotion: window.localStorage.getItem(`${prefix}categoryIsMotion`) === 'true',
  },

  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === '/system/category') {
  ...
  effects: {
    ...
    * multiDelete ({ payload }, { call, put }) {
      const data = yield call(categoriesService.remove, payload)
      if (data.success) {
  ...
  reducers: {
    ...
    switchIsMotion (state) {
      window.localStorage.setItem(`${prefix}categoryIsMotion`, !state.isMotion)
